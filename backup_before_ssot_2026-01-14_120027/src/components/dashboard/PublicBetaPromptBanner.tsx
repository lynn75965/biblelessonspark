 ...
 * 
 * SSOT COMPLIANCE:
 * - All UI text from betaEnrollmentConfig.ts
...
import { 
  BETA_ENROLLMENT_CONFIG, 
  shouldPromptOrgEnrollment 
} from '@/constants/betaEnrollmentConfig';
...
export function PublicBetaPromptBanner({ 
  ...
  const config = BETA_ENROLLMENT_CONFIG.dashboardPrompt;
  const messages = BETA_ENROLLMENT_CONFIG.messages;

  // Check if banner should be shown
  useEffect(() => {
    // Check platform mode and org status
    ...
    try {
      ...
      if (memberError) {
        ...
        if (memberError.code === '23505') {
          toast({
            title: messages.alreadyEnrolled.title,
            description: messages.alreadyEnrolled.description,
          });
          setIsVisible(false);
          return;
        }
      ...
      toast({
        title: messages.enrollmentError.title,
        description: messages.enrollmentError.description,
        variant: 'destructive',
      });
    } finally {
