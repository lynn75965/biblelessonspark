...
for it was ever added to supabase/config.toml -- confirmed absent
...
permanently empty. Fixed by adding the missing verify_jwt=false stanza
to config.toml and redeploying (config.toml's verify_jwt setting is
applied at deploy time, confirmed empirically by re-running the same
reachability test post-deploy: org-stripe-webhook now returns its own
400 "No Stripe signature" instead of the platform's 401). In the same
pass, wired stripe_events -- a table that already existed, fully
shaped (event_id text with a UNIQUE constraint already in place), zero
rows, referenced by zero code anywhere -- as a real idempotency ledger
on BOTH webhooks: insert the event_id immediately after signature
verification succeeds and before any state change; a UNIQUE violation
means a redelivery, return 200 and process nothing further. This
closes the redelivery exposure the same-day Stripe diagnostic had
documented in detail: duplicate organization creation on a repeated
...
billing-period anchor shifting to the redelivery's own wall-clock time
