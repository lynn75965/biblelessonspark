...
**2. org-stripe-webhook's 175-day unreachability CONFIRMED -- NOT FIXED
THIS SESSION.** It inherits the platform default verify_jwt=true
because no stanza for it has ever been added to supabase/config.toml
-- confirmed absent across that file's entire tracked git history back
to its earliest commit (September 2025), and the function itself was
created 2026-01-30 (commit b81f581). Since Stripe authenticates
webhook deliveries via the Stripe-Signature header only, never a
bearer token, every real delivery is rejected at the platform edge
with 401 UNAUTHORIZED_NO_AUTH_HEADER before the function's own manual
HMAC signature check ever runs. Proven, not inferred: a live
reachability test (a POST carrying no Authorization header and a
syntactically well-formed but deliberately invalid Stripe-Signature --
safe, since an invalid signature cannot cause any state change even if
the request reached the handler) returned the platform's own 401
against org-stripe-webhook and, as a control, the function's own 400
"Invalid signature" against the sibling stripe-webhook (which has
...
A fix was PROPOSED and Phase A of it was approved and executed --
Phase B and Phase C were NOT. Phase A (delete the three
tutorial-fixture organizations) completed and was verified in full,
see item 1 above. Lynn then redirected to the beta-status work below
before ruling on Phase B. Phase B (add verify_jwt=false to
supabase/config.toml; wire stripe_events as an idempotency ledger on
both webhooks) and Phase C (redeploy and re-verify reachability) were
NEVER EXECUTED. An earlier draft of this entry incorrectly stated the
fix had shipped -- corrected here after Lynn caught the error on
read-back of the committed text. Re-verified against live state a
second time while making this correction: supabase/config.toml still
carries no org-stripe-webhook stanza; the function still returns 401
UNAUTHORIZED_NO_AUTH_HEADER at the platform edge; it has NEVER
processed a single Stripe event in its entire 175-day existence;
stripe_events (a table that already existed, fully shaped -- event_id
text with a UNIQUE constraint already in place -- but referenced by
zero code anywhere) is still 0 rows; and NEITHER webhook has any
idempotency protection, so the full redelivery exposure documented in
the same-day Stripe diagnostic remains open on both stripe-webhook and
org-stripe-webhook: duplicate organization creation on a repeated
checkout.session.completed, lessons_used_this_period reset to 0 on a
repeated invoice.paid, doubled bonus_lessons on a repeated lesson-pack
purchase event, duplicate org_onboarding_purchases rows, and the
billing-period anchor shifting to a redelivery's own wall-clock time
rather than the original event's. stripe_events itself was created via
...
**THIS IS THE NEXT REQUIRED SESSION. It blocks selling org tiers** --
until verify_jwt=false is added to config.toml and the function is
redeployed, no org-subscription Stripe event of any kind (checkout,
renewal, cancellation, invoice) can ever reach org-stripe-webhook,
regardless of how the Stripe Dashboard's webhook endpoint is
configured.
