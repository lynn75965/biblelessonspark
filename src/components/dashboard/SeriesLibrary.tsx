...
export function SeriesLibrary() {
  ...
  const getStatusBadge = (series: { status: string; lesson_summaries: any[]; total_lessons: number }) => {
    ...
    if (series.status === SERIES_STATUSES.ABANDONED) return (
    ...
    <div className="space-y-6">
      <Card className="bg-gradient-card border-border/50">
        <CardHeader>
          ...
          <CardDescription>Browse your teaching series and export them as curriculum documents</CardDescription>
        </CardHeader>
      </Card>
            ...
            return (
              <Card key={series.id} className="group hover:shadow-glow transition-all duration-normal bg-gradient-card">
                <CardHeader className="p-4 sm:p-6">
                  ...
                  <div className="flex flex-wrap gap-1.5 sm:gap-2 mt-3">{getStatusBadge(series)}</div>
                  <div className="mt-3">
                ...
                <CardContent className="p-4 sm:p-6 pt-0">
                  <div className="flex items-center justify-between text-xs text-muted-foreground mb-4">
                    <span className="flex items-center gap-1">
                      <BookOpen className="h-3 w-3" />Created {formatDate(series.created_at)}
                    </span>
                  </div>
        ...
        <Card className="bg-gradient-card border-border/50">
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <div className="space-y-2 text-center">
              ...
              <p className="text-muted-foreground max-w-md">Create a teaching series from the Build Lesson tab by selecting "Part of Series" to plan a multi-week study.</p>
            </div>
          </CardContent>
        </Card>
      )}
