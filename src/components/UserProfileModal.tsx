  ...
  const loadProfile = async () => {
    ...
    try {
      const { data: profile, error } = await supabase
        .from('profiles')
        .select('full_name, preferred_language, default_bible_version, theology_profile_id, organization_role, organization_id, audience_profile')
        .eq('id', user.id)
        .single();
      ...
      setAudienceRole(ap.role);
      setAudienceAssembly(ap.assembly);
      setAudienceParticipant(ap.participant);
      ...
      <DialogContent className="sm:max-w-[500px] max-h-[90vh] overflow-y-auto">
        ...
        {initialLoading ? (
          ...
          <div className="grid gap-4 py-4">
            ...
            {/* -- MINISTRY CONTEXT (Audience Triad) --------------- */}
            <div className="grid gap-1">
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="grid gap-1">
                ...
                <Select
                  value={audienceRole}
                  onValueChange={(value) => setAudienceRole(value)}
                >
              ...
              {/* Gathering */}
              <div className="grid gap-1">
                <Label className="text-xs">Gathering</Label>
                <Select
                  value={audienceAssembly}
                  onValueChange={(value) => setAudienceAssembly(value)}
                >
                  <SelectTrigger className="h-9">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    {AUDIENCE_ASSEMBLIES.map(a => (
              ...
              <div className="grid gap-1">
                ...
                <Select
                  value={audienceParticipant}
                  onValueChange={(value) => setAudienceParticipant(value)}
                >
                  <SelectTrigger className="h-9">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    {AUDIENCE_PARTICIPANTS.map(p => (
