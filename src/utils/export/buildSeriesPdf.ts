...
export async function buildSeriesPdf(
  ...
  function renderSectionHeading(text: string): void {
    // Use scheme heading color for section headings
    const [r, g, b] = hexToRgb(scheme.heading);
  ...
  function renderSubheading(text: string): void {
    // Use scheme heading color for subheadings
    const [r, g, b] = hexToRgb(scheme.heading);
    doc.setFont(EXPORT_SPACING.fonts.pdf, 'bold');
  ...
  // Cover title -- scheme heading color
  const [tr, tg, tb] = hexToRgb(scheme.heading);
  doc.setFont(EXPORT_SPACING.fonts.pdf, 'bold');
  ...
  // Cover subtitle -- scheme accent color
  ...
  const [hr1, hg1, hb1] = hexToRgb(scheme.accent);
  ...
  for (let i = 0; i < lessons.length; i++) {
    ...
    // Chapter title -- scheme heading color
    const [ct_r, ct_g, ct_b] = hexToRgb(scheme.heading);
    doc.setFont(EXPORT_SPACING.fonts.pdf, 'bold');
    ...
    // Horizontal rule -- scheme accent color
    const [hr2, hg2, hb2] = hexToRgb(scheme.accent);
