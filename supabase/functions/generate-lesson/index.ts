...
// 🆕 THEOLOGY PROFILE IMPORTS
// Note: These would normally be imported from shared files, but in Deno Edge Functions
// we need to inline them or create them in _shared folder
// For now, we'll inline the minimal types and functions we need
...
serve(async (req) => {
  ...
  try {
    ...
    // 🆕 4. GET USER PREFERENCES (Language AND Theology)
    ...
    // 🆕 5. BUILD COMPREHENSIVE PROMPT WITH THEOLOGY PROFILE INTEGRATION
...
8. Additional Resources/Notes for Teacher`;
    ...
    console.log("Calling Anthropic API with theology profile:", theologyProfile.confession_label);
    ...
    console.log("✅ Lesson generated successfully with theology profile:", theologyProfile.id);
